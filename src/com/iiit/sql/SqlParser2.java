...
public class SqlParser2 {
	...
	public static void main(String args[]) 
	{
        ...
        try {
			...
			System.out.println(meta.tables);
		} catch (IOException e) {
	...
	static void processQuery(ZqlParser p)
	{
		try {
	...
	static boolean processFrom(Vector from)
	{
		...
		for(i = 0;i<from.size();i++)
		{
			...
			{
				printError(from.get(i)+ " table not found");
				return false;
			}
		}
		...
		try {
			...
			finalTable = table; 
//			System.out.println(table.colMap);
	...
	public static void processSelect(Vector<ZSelectItem> select, boolean isDistinct)
	{
		...
		if(select.size()==1)
		{
			...
			{
				String name;
				if(select.get(0).getSchema()==null)
				{
		...
		}else
		{
			...
			{
				String name;
				if(select.get(i).getSchema()==null)
				{
		...
		if(aggFlag==true)
		{
			...
			for(int i = 0;i<aggResult.size();i++)
			{
				...
				{
					float val = Float.parseFloat((String) finalTable.tab.get(0).get(i));
...
//						System.out.println("Size after remove"+temp.size());
	...
	private static boolean processWhere(ZExpression where)
	{
		...
		if(where.getOperator().equals("AND") || where.getOperator().equals("OR"))
		{
			...
			String op2 = exp1.getOperand(1).toString();
			...
			op2 = exp2.getOperand(1).toString();
			op = exp2.getOperator();
		...
		}else
		{
			...
			String op2 = where.getOperand(1).toString();
	...
	private static void filterTableForWhere(Vector<Exp> exps,String logical)
	{
	...
	private static boolean filterColumnsForSelect(Vector columns)
	{
	...
	private static float computeAggregate(int colIndex,String fname) {
		// TODO Auto-generated method stub
		
		System.out.println("Agg fun"+fname);
